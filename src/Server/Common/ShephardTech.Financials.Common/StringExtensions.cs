...
namespace ShephardTech.Financials.Common
{
    ...
    {
        /// <summary>
        /// Convert any specified string to Base64 String.
        /// </summary>
        ...
        public static bool isValidNigeriaGSM(this string phoneNumber)
        {
            bool isValid = false;
            ...
            string[] validFirstFourXters = { "0708", "0703", "0703", "0701","0702",
                "0808", "0803", "0803", "0801","0802",
             "0908", "0903", "0903", "0901","0902"};//Add to the array if desired

            for (int i = 0; i <= validFirstFourXters.Length - 1; i++)
            {
                if (phoneNumber.Left(4) == validFirstFourXters[i])
                {
                    isValid = true;
                    break;
                }
        ...
        public static string RandomString(int length, bool lowerCase = true, bool upperCase = true, bool numbers = true, bool symbols = false)
        {
            ...
            return stringGenerator.Next(length, lowerCase, upperCase, numbers, symbols);
        }
        ...
        /// <param name="data">String date</param>
        /// <param name="dateFormat">date format example dd/MM/yyyy HH:mm:ss</param>
        /// <returns>DateTime if successful else returns default date  False if is valid System.DateTime</returns>
        public static DateTime ToDateTime(this string data, string dateFormat)
        {
            var dateSplit = data.Replace("-", "/").Split("/");
            _ = int.TryParse(dateSplit[2], out int yr);
            _ = int.TryParse(dateSplit[1], out int mn);
            _ = int.TryParse(dateSplit[0], out int day);

            return new DateTime(yr, mn, day);
        }

        public static int RandomNumber(int lastNum)
        {
            var template = "0000000000";

            return int.Parse($"{template}{(lastNum + 1).ToString()}".Right(10));

        }

        private static readonly string[] _validExtensions = { ".png", ".jpg", ".jpeg", ".bmp", ".webp", ".bitmap", ".gif" };
